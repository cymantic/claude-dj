...
server.tool("play_song", "Search and play a song on Spotify with context for learning preferences", {
    query: QuerySchema,
...
server.tool("play_snippet", "Play a short music snippet that auto-fades after duration", {
    query: QuerySchema,
    context: ContextSchema,
...
server.tool("thumbs_down", "Mark the current track as bad for its context - won't play again for this context", {
    reason: ReasonSchema,
}, async ({ reason }) => {
    const trackList = tracklist.loadTrackList();
...
server.tool("promote_track", "Boost the current track for a mood/context - makes it more likely to play", {
    context: ContextSchema,
...
server.tool("set_volume", "Set Spotify volume level", {
...
server.tool("cancel_snippet", "Cancel the auto-fade timer and let the current song keep playing", {}, async () => {
...
server.tool("resume", "Resume Spotify playback", {}, async () => {
    spotify.resume();
    return {
        content: [
            {
...
server.tool("get_status", "Get current Spotify playback status", {}, async () => {
    const status = spotify.getStatus();
    return {
        content: [
            {
...
server.tool("get_moods", "Get list of known mood contexts and how many tracks each has", {}, async () => {
    const trackList = tracklist.loadTrackList();
